...
meta_both <- meta_both %>% split(meta_both$Country) %>% lapply(., function(x){
  x$n = 1:nrow(x)
  x$new_name = str_c(x$Country, "_", x$n)
  x}) %>% do.call(rbind, .)
...
ft_ITS_clean_wk_s <- ft_ITS_clean_wk %>% rownames_to_column(var = "id") %>% 
  left_join(taxa_ITS, by = "id") %>% 
  mutate(taxa_label = str_c(Phylum, Species_l, sep = ";")) %>% 
  select(-c(id,Kingdom:Species_l)) %>% 
...
ft_tmp <- ft_ITS_clean_wk_s %>% rownames_to_column(var = "taxa_label") %>% 
  ...
  left_join(meta_both, by = c("name" = "SampleID_clean")) %>% 
  arrange(Continent, Country) 

sample_ord <- meta_both %>% arrange(Continent, Country, SampleID_clean) %>% pull(new_name)
sample_col <- meta_both %>% arrange(Continent, Country, SampleID_clean) %>% 
  mutate(color = Country_color[Country]) %>% pull(color)

col_vector = c(brewer.pal(12, "Set3"), brewer.pal(9, "Set1"))
cols = c("gray30", col_vector[1:20])

g <- ggplot(ft_tmp %>% mutate(name = factor(new_name, levels = sample_ord, ordered = TRUE)),
            aes(new_name, value)) +
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
    ...
    axis.text.x = element_markdown(size=12, face='bold',angle=90,vjust = 0.5, hjust = 1, color = sample_col),
...
ra_ITS_clean_wk_s <- ra_ITS_clean_wk_s %>% select(-c(id,Kingdom:Species_l)) %>% 
  select(taxa_label ,dplyr::everything()) %>% 
  group_by(taxa_label) %>%
  summarise(across(everything(), sum),
            .groups = 'drop') %>% 
...
g <- ggplot(ra_tmp %>% mutate(name = factor(new_name, levels = sample_ord, ordered = TRUE)),
            aes(new_name, value)) +
  geom_col(aes(fill = taxa_label), position = "stack") +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_manual(values = cols) + 
...
## Genus
#### read count
ft_ITS_clean_wk_g <- ft_ITS_clean_wk %>% rownames_to_column(var = "id") %>% 
  left_join(taxa_ITS, by = "id") %>% 
  mutate(taxa_label = str_c(Phylum, Genus_l, sep = ";")) %>% 
  select(-c(id,Kingdom:Species_l)) %>% 
  select(taxa_label ,dplyr::everything()) %>% 
  group_by(taxa_label) %>%
  summarise(across(everything(), sum),
...
ft_ITS_clean_wk_g <- ft_ITS_clean_wk_g[, colSums(ft_ITS_clean_wk_g)>0]

top_id <- (rowMeans(ft_ITS_clean_wk_g) %>% sort(decreasing = TRUE) %>% names())[1:15]

ft_tmp <- ft_ITS_clean_wk_g %>% rownames_to_column(var = "taxa_label") %>% 
  mutate(taxa_label = ifelse(taxa_label %in% top_id, taxa_label, "other")) %>% 
  group_by(taxa_label) %>%
  summarise(across(everything(), sum),
            .groups = 'drop') %>% 
...
ggsave(filename = paste0(temp_out, "read_count_fungi_genus_per_sample.pdf"), device = cairo_pdf, 
       plot = g, width = 14, height = 8, units = "in", dpi = 300)  
...
ra_ITS_clean_wk_g <- ra_ITS_clean_wk %>% rownames_to_column(var = "id") %>% 
  left_join(taxa_ITS, by = "id") %>% 
  mutate(taxa_label = str_c(Phylum, Genus_l, sep = ";"))

ra_ITS_clean_wk_g <- ra_ITS_clean_wk_g %>% select(-c(id,Kingdom:Species_l)) %>% 
  select(taxa_label ,dplyr::everything()) %>% 
  group_by(taxa_label) %>%
  summarise(across(everything(), sum),
            .groups = 'drop') %>% 
...
top_id <- (rowMeans(ra_ITS_clean_wk_g) %>% sort(decreasing = TRUE) %>% names())[1:15]

ra_tmp <- ra_ITS_clean_wk_g %>% rownames_to_column(var = "taxa_label") %>% 
  mutate(taxa_label = ifelse(taxa_label %in% top_id, taxa_label, "other")) %>% 
  group_by(taxa_label) %>%
  summarise(across(everything(), sum),
            .groups = 'drop') %>% 
...
g <- ggplot(ra_tmp %>% mutate(new_name = factor(new_name, levels = sample_ord, ordered = TRUE)),
            aes(new_name, value)) +
  geom_col(aes(fill = taxa_label), position = "stack") +
  scale_fill_manual(values = cols) + 
  scale_y_continuous(expand = c(0,0)) +
...
ggsave(filename = paste0(temp_out, "RA_fungi_genus_per_sample.pdf"), device = cairo_pdf, 
       plot = g, width = 14, height = 8, units = "in", dpi = 300)  
...
```{r}
options(scipen=999)
dat <- data.frame(taxa = rownames(ra_ITS_clean_wk_s), pre = rowSums(ra_ITS_clean_wk_s>0), abundance = rowMeans(ra_ITS_clean_wk_s))
write.table(dat, file = paste0(temp_out, "prev_abn_fungi_species.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)

dat <- data.frame(taxa = rownames(ra_ITS_clean_wk_g), pre = rowSums(ra_ITS_clean_wk_g>0), abundance = rowMeans(ra_ITS_clean_wk_g))
write.table(dat, file = paste0(temp_out, "prev_abn_fungi_genus.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
