...
ft_its_wk_g_filter <- ft_its_wk_g_filter  %>% as.data.frame() %>% 
  rownames_to_column(var = "SampleID_clean") %>% arrange(SampleID_clean) %>% filter(SampleID_clean != "BR34.1")
...
mt_wk <- ft_16s_wk_g_filter %>% column_to_rownames(var = "SampleID_clean") %>% as.matrix()
set.seed(2025)
...
save(res, file = paste0(temp_out,"network_20250320.Rdata"))
```

### plot
```{r}
load(file = paste0(temp_out,"network_20250320.Rdata"))
...
for (nn in names(res)){
  ...
  for (ll in c('fr','circle')){
    ...
    ggsave(g, file = paste0(temp_out, "sparcc_",nn,"_",ll,"_pval0.05_R6.pdf"), 
           device = pdf)
  }
}
...
ft_16s_wk_g_filter <- ft_16s_wk_g_filter %>% filter(SampleID_clean %in% ft_its_wk_g_filter$SampleID_clean)
...
save(res, file = paste0(temp_out,"network_20250320_same_size.Rdata"))
```

### plot
```{r}
load(file = paste0(temp_out,"network_20250320_same_size.Rdata"))
k_color = c("Fungi"="#fc8d62", "Bacteria"="#8da0cb", "Archaea"="#66c2a5")
...
for (nn in names(res)){
  ...
  for (ll in c('fr','circle')){
    ...
    ggsave(g, file = paste0(temp_out, "sparcc_new_same_size_",nn,"_",ll,"_pval0.05_R6.pdf"), 
           device = pdf)
  }
}
