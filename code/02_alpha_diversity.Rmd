...
g <- ggplot(dat, aes(heat_inactivated, value)) + 
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = paste0(temp_out, "alpha_by_heat_linked.pdf"), device = cairo_pdf, 
         plot = g, width = 9, height = 7, units = "in", dpi = 300) 
...
```{r}
dat <- dat_alpha %>% filter(!is.na(DNA_Conc_ng_uL)) %>% 
  dplyr::select(SampleID, DNA_Conc_ng_uL, Faith_PD:Pielou_evenness) %>% 
  pivot_longer(Faith_PD:Pielou_evenness, names_to = "Metrics", values_to = "value")

g <- ggplot(dat, aes(DNA_Conc_ng_uL,value)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  stat_regline_equation(label.x.npc = 0,label.y.npc = 0.2) + 
  stat_cor(label.x.npc = 0, label.y.npc = 0.01) + 
  scale_x_log10() + 
  scale_y_log10() +  
...
dat <- dat_alpha %>% filter(!is.na(PCRConc)) %>% 
  dplyr::select(SampleID, PCRConc, Faith_PD:Pielou_evenness) %>% 
  pivot_longer(Faith_PD:Pielou_evenness, names_to = "Metrics", values_to = "value")
