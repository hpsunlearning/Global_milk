...
Country_color <- c(RColorBrewer::brewer.pal(12, "Set3"), RColorBrewer::brewer.pal(4, "Dark2"))
names(Country_color) <- c("Guinea Ecuatorial", "Kenya", "Senegal", "South Sudan", "Tanzania",
                          "Ecuador", "ElSalvador", "Mexico", "Peru", "Puerto Rico", "United States",
                          "Austria", "Germany", "Netherlands", "Norway", "Spain") 
#ggplot(data.frame(country = names(Country_color), y = rep(1, 16))) + geom_col(aes(x=country, y=y, color=country, fill=country)) + scale_color_manual(values = Country_color, aesthetics = c("color","fill"))
```
...
#1 project info for different samples
meta_all <- meta_all %>% 
  mutate(Project = case_when(Country == "United States" & str_detect(SampleID, "^Cov") ~ "Covid Mom",
                             Country == "United States" & str_detect(SampleID, "^UP") ~ "Upside",
                             Country == "United States" & str_detect(SampleID, "^Fam") ~ "Family",
                             Country == "United States" & str_detect(SampleID, "^\\d+") ~ "Milkbank",
                             Country == "Spain" & heat_inactivated == "Y" ~ "Covid Spain"))
#2 heat inactivation compare info
...
prev <- contamdf %>% filter(contaminant) %>% rownames()
...
taxa %>% filter(id %in% unique(unlist(lst))) %>% 
...
rm(contamdf, contam_id, a1, a2, a3, a4, prev, combined, minimum, either, both, lst)
```
...
g <- ggplot(dat %>% filter(heat_compare == "N"), 
            aes(Country, Seq_count_clean)) + 
...
g <- ggplot(dat %>% filter(heat_compare == "N"), 
            aes(DNA_Conc_ng_uL, Seq_count_clean)) + 
  geom_point(aes(color = Country)) + 
  scale_x_log10() + 
  scale_y_log10() +
  scale_color_manual(values = Country_color, aesthetics = c("color","fill")) + 
  geom_smooth(method = "lm") + 
  stat_regline_equation(label.x.npc = 0.6,label.y.npc = 0.3) + 
  stat_cor(label.x.npc = 0.6, label.y.npc = 0.2) + 
  labs(x = expression("DNA concentration (ng/"*mu*"L)"), y = "Clean sequence count") + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     #legend.position="none",
                     panel.background = element_rect(fill = NA),
                     strip.text = element_text(size=12,color="black",face='bold'),
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/clean_reads_count_by_DNA_conc.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```

## depth by PCR concentration
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat %>% filter(heat_compare == "N"), 
  ...
  stat_regline_equation(label.x.npc = 0.6,label.y.npc = 0.3) + 
  stat_cor(label.x.npc = 0.6, label.y.npc = 0.2) + 
  labs(x = expression("PCR concentration (ng/"*mu*"L)"), y = "Clean sequence count") + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     #legend.position="none",
                     panel.background = element_rect(fill = NA),
                     strip.text = element_text(size=12,color="black",face='bold'),
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/clean_reads_count_by_PCR_conc.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```

## depth by heat_inactivation 
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat,
            aes(heat_inactivated, Seq_count_clean)) + 
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/clean_reads_count_by_heat.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)

# heat_inactivation individual
wdat <- dat %>% 
              select(Original_SampleID, heat_inactivated, DNA_Conc_ng_uL, Seq_count_clean) %>% 
              pivot_wider(id_cols = Original_SampleID, 
                          names_from = heat_inactivated, names_glue = "{.value}_{heat_inactivated}",
                          values_from = c(DNA_Conc_ng_uL, Seq_count_clean)) 

g <- ggplot(wdat,
            aes(Seq_count_clean_N, Seq_count_clean_Y)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/clean_reads_count_by_heat_individual.pdf",
       device = cairo_pdf, plot = g, width = 7, height = 5, units = "in", dpi = 300)

```

## depth by extraction method
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat,
            aes(Extraction_method, Seq_count_clean)) + 
  geom_boxplot(aes(color = Extraction_method)) + 
  geom_point(aes(color = Extraction_method), position = position_dodge2()) + 
  stat_compare_means() + 
  labs(x="", y="Clean sequence count") + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     legend.position="none",
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/clean_reads_count_by_method.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```

## DNA Conc by heat_inactivation
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat,
            aes(heat_inactivated, DNA_Conc_ng_uL)) + 
  geom_boxplot(aes(color = heat_inactivated)) + 
  geom_point(aes(color = heat_inactivated)) + 
  geom_path(aes(group = Original_SampleID), alpha = 0.6, color = "gray") +
  stat_compare_means() + 
  scale_x_discrete(labels = c("N"="No heat", "Y"="Heat inactivated")) + 
  labs(x="", y=expression("DNA concentration (ng/"*mu*"L)")) + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     legend.position="none",
                     panel.background = element_rect(fill = NA),
                     strip.text = element_text(size=12,color="black",face='bold'),
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/DNAconc_by_heat.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
...
g <- ggplot(wdat,
            aes(DNA_Conc_ng_uL_N, DNA_Conc_ng_uL_Y)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  ...
  scale_x_log10() + 
  scale_y_log10() +  
  labs(x="No heat", y="Heat inactivated", title = expression("DNA concentration (ng/"*mu*"L)")) + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     legend.position="none",
                     panel.background = element_rect(fill = NA),
                     strip.text = element_text(size=12,color="black",face='bold'),
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/DNAconc_by_heat_individual.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```

## DNA Conc by extraction method
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat,
            aes(Extraction_method, DNA_Conc_ng_uL)) + 
  geom_boxplot(aes(color = Extraction_method)) + 
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/DNAconc_by_method.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```


## PCR Conc by heat_inactivation 
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat,
            aes(heat_inactivated, PCRConc)) + 
  geom_boxplot(aes(color = heat_inactivated)) + 
  geom_point(aes(color = heat_inactivated)) + 
  geom_path(aes(group = Original_SampleID), alpha = 0.6, color = "gray") +
  stat_compare_means() + 
  scale_x_discrete(labels = c("N"="No heat", "Y"="Heat inactivated")) + 
  labs(x="", y=expression("PCR concentration (ng/"*mu*"L)")) + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     legend.position="none",
                     panel.background = element_rect(fill = NA),
                     strip.text = element_text(size=12,color="black",face='bold'),
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/PCRconc_by_heat.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)

# heat_inactivation individual
wdat <- dat %>% 
              select(Original_SampleID, heat_inactivated, PCRConc, Seq_count_clean) %>% 
              pivot_wider(id_cols = Original_SampleID, 
                          names_from = heat_inactivated, names_glue = "{.value}_{heat_inactivated}",
                          values_from = c(PCRConc, Seq_count_clean)) 

g <- ggplot(wdat,
            aes(PCRConc_N, PCRConc_Y)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  stat_regline_equation(label.x.npc = 0.6,label.y.npc = 0.2) + 
  stat_cor(label.x.npc = 0.6, label.y.npc = 0.1) + 
  scale_x_log10() + 
  scale_y_log10() +  
  labs(x="No heat", y="Heat inactivated", title = expression("PCR concentration (ng/"*mu*"L)")) + 
  theme_bw() + theme(#aspect.ratio = 0.8,
                     legend.position="none",
                     panel.background = element_rect(fill = NA),
                     strip.text = element_text(size=12,color="black",face='bold'),
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/PCRConc_by_heat_individual.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```

## PCR Conc by extraction method
```{r}
dat <- phylo_decontam_clean@sam_data@.Data %>% as.data.frame()
names(dat) <- phylo_decontam_clean@sam_data@names
dat$SampleID <- phylo_decontam_clean@sam_data@row.names
...
g <- ggplot(dat,
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
                     ...
                     legend.title = element_text(size=12,color="black",face='bold'),
                     title = element_text(size=12,color="black",face='bold'))
ggsave(filename = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/PCRconc_by_method.pdf", device = cairo_pdf, 
         plot = g, width = 7, height = 5, units = "in", dpi = 300)
```
...
dat <- dat %>% 
  mutate(Project2 = case_when(Country == "United States" & str_detect(SampleID, "^C") ~ "Covid Mom",
                             Country == "United States" & str_detect(SampleID, "^UP") ~ "Upside",
                             Country == "United States" & str_detect(SampleID, "^F") ~ "Family",
                             Country == "United States" & str_detect(SampleID, "^\\d+") ~ "Milkbank",
                             Country == "Spain" & heat_inactivated == "Y" ~ "Covid Spain",
                             Country == "Puerto Rico" ~ "PR",
                             TRUE ~ "Global")) %>% 
  arrange(Country, Project, SampleID)

a <- dat %>% select(SampleID, Extraction_method, heat_inactivated, Country, Project2) %>% 
  group_by(Extraction_method, heat_inactivated, Country, Project2) %>% 
  summarise(N = n()) %>% ungroup() %>% 
  pivot_wider(id_cols = c(Project2, Extraction_method, heat_inactivated), 
              names_from = "Country",
              values_from = "N") 
...
a <- dat %>% select(SampleID, Extraction_method, heat_inactivated, Country) %>% 
  group_by(Extraction_method, heat_inactivated, Country) %>% 
  summarise(N = n()) %>% ungroup() %>% 
  pivot_wider(id_cols = c(Extraction_method, heat_inactivated), 
              names_from = "Country",
              values_from = "N")
write.csv(a, file = "~/Dropbox/Rutgers/20220606_RUN41_milk/Data/output/by_country_N.csv",
          row.names = FALSE)
```

## subset to each method for depth and conc compare
```{r}
