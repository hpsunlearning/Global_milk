...
taxa <- taxa %>% 
  mutate(Class_l = case_when(Class == "c_" ~ str_c(Phylum, Class, sep = ";"),
                             TRUE ~ Class),
         Order_l = case_when(Order == "o_" ~ str_c(Class_l, Order, sep = ";"),
                             str_detect(Order, "uncultured") ~ str_c(Class_l, Order, sep = ";"),
                             TRUE ~ Order),
         Family_l = case_when(Family == "f_" ~ str_c(Order_l, Family, sep = ";"),
                              str_detect(Family, "uncultured") ~ str_c(Order_l, Family, sep = ";"),
                              TRUE ~ Family),
         Genus_l = case_when(Genus == "g_" ~ str_c(Family_l, Genus, sep = ";"),
                             str_detect(Genus, "uncultured") ~ str_c(Family_l, Genus, sep = ";"),
                             TRUE ~ Genus),
         Species_l = case_when(Species == "s_" ~ str_c(Genus_l, Species, sep = ";"),
                               str_detect(Species, "uncultured|metagenome") ~ str_c(Genus_l, Species, sep = ";"),
                               TRUE ~ Species))
...
meta_global %>% summarise(my_summarize2(Seq_count)) %>% print()
meta_global %>% summarise(my_summarize2(decontam_cleaned)) %>% print()


sum(!is.na(meta_global_rf$Maternal_age))
sum(!is.na(meta_global_rf$Maternal_weight_kg))
sum(!is.na(meta_global_rf$Maternal_height_cm))
sum(!is.na(meta_global_rf$Birth_mode))
sum(!is.na(meta_global_rf$Time_post_partum_day))

my_summarize3 = function(x){
  data.frame(
    N = n(),
    N.nna = sum(!is.na(x)),
    Med = median(x, na.rm = T),
...
meta_global %>% 
  #group_by(Country) %>% 
  summarise(my_summarize3(Maternal_age)) %>% 
  #write_csv(.,file = paste0(temp_out,"Global_samples_Maternal_age.csv"))

meta_global %>% group_by(Country) %>% 
  summarise(my_summarize3(Maternal_weight_kg)) %>% 
  write_csv(.,file = paste0(temp_out,"Global_samples_Maternal_weight_kg.csv"))

meta_global %>% group_by(Country) %>% 
  summarise(my_summarize3(Time_post_partum_day)) %>% 
  write_csv(.,file = paste0(temp_out,"Global_samples_Time_post_partum_day.csv"))

meta_global %>% group_by(Country) %>% 
  count(Birth_mode) %>% pivot_wider(id_cols = Country, names_from = Birth_mode, values_from = n) %>% 
  write_csv(.,file = paste0(temp_out,"Global_samples_Birth_mode.csv"))
...
## Maternal_age
alpha_lm_coef <- map_dfr(metric, function(a){
  fit <- lm(as.formula(paste0(a, "  ~ Maternal_age" )), data = dat)
  anova_fit <- anova(fit)
  sum_fit <- summary(fit)
  effect_fit <- effectsize(anova_fit, "omega")
  res_coef <- sum_fit$coefficients %>% as.data.frame() %>% 
...
alpha_lm_coef$pvalue = round(alpha_lm_coef$`Pr(>|t|)`, digits = 4)
write.csv(alpha_lm_coef, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_coef_Maternal_age.csv"))
...
alpha_lm_omega$pvalue = round(alpha_lm_omega$`Pr(>F)`, digits = 4)
write.csv(alpha_lm_omega, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_omega_Maternal_age.csv"))

## Birthmode
alpha_lm_coef <- map_dfr(metric, function(a){
  fit <- lm(as.formula(paste0(a, "  ~ Birth_mode" )), data = dat)
  anova_fit <- anova(fit)
  sum_fit <- summary(fit)
  effect_fit <- effectsize(anova_fit, "omega")
  res_coef <- sum_fit$coefficients %>% as.data.frame() %>% 
...
alpha_lm_coef$pvalue = round(alpha_lm_coef$`Pr(>|t|)`, digits = 4)
write.csv(alpha_lm_coef, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_coef_Birth_mode.csv"))
...
alpha_lm_omega$pvalue = round(alpha_lm_omega$`Pr(>F)`, digits = 4)
write.csv(alpha_lm_omega, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_omega_Birth_mode.csv"))

## Maternal_weight_kg
alpha_lm_coef <- map_dfr(metric, function(a){
  fit <- lm(as.formula(paste0(a, "  ~ Maternal_weight_kg" )), data = dat)
  anova_fit <- anova(fit)
  sum_fit <- summary(fit)
  effect_fit <- effectsize(anova_fit, "omega")
  res_coef <- sum_fit$coefficients %>% as.data.frame() %>% 
...
alpha_lm_coef$pvalue = round(alpha_lm_coef$`Pr(>|t|)`, digits = 4)
write.csv(alpha_lm_coef, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_coef_Maternal_weight_kg.csv"))
...
alpha_lm_omega$pvalue = round(alpha_lm_omega$`Pr(>F)`, digits = 4)
write.csv(alpha_lm_omega, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_omega_Maternal_weight_kg.csv"))

## All
alpha_lm_coef <- map_dfr(metric, function(a){
  fit <- lm(as.formula(paste0(a, "  ~ Birth_mode + Maternal_weight_kg + Maternal_age +  Time_post_partum_day + Country" )), data = dat)
  anova_fit <- anova(fit)
  sum_fit <- summary(fit)
  effect_fit <- effectsize(anova_fit, "omega")
  res_coef <- sum_fit$coefficients %>% as.data.frame() %>% 
...
alpha_lm_coef$pvalue = round(alpha_lm_coef$`Pr(>|t|)`, digits = 4)
write.csv(alpha_lm_coef, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_coef_All.csv"))
...
alpha_lm_omega$pvalue = round(alpha_lm_omega$`Pr(>F)`, digits = 4)
write.csv(alpha_lm_omega, row.names = FALSE,
          file = paste0(temp_out,"alpha_lm_omega_All.csv"))
...
stat.test <- wdat %>% group_by(Metrics) %>% 
  ...
  rstatix::adjust_pvalue(method = "fdr") %>%
  rstatix::add_significance("p.adj") %>% 
  rstatix::add_xy_position(x = "Continent", dodge = 0.8, scales = "free_y") %>% 
  filter(p.adj.signif != "ns")
...
g <- ggplot(wdat, aes(Continent, value)) + 
  ...
  theme_bw() + theme(#aspect.ratio = 0.8,
    ...
    legend.title = element_text(size=12,color="black",face='bold'),
    title = element_text(size=12,color="black",face='bold'))
ggsave(filename = paste0(temp_out, "alpha_by_Continent_full_pvalue.pdf"), device = cairo_pdf, 
       plot = g, width = 14, height = 10, units = "in", dpi = 300) 
...
for(dd in c("uu","wu","bray","jaccard")){
  ...
  # ## postpartum 
  # wwmeta <- wmeta %>% filter(!is.na(Time_post_partum_day))
  # wwdist <- dist_subset(wdist, wwmeta$SampleID )
  # disper <- betadisper(wwdist, wwmeta$Time_post_partum_day)
  # disper_ova <- anova(disper)
  # disper_HSD <- TukeyHSD(disper)
  # write.csv(disper_ova, file = paste0(temp_out,"disper_anova_", dd, "_postpartum.csv"), quote = F)
  # write.csv(disper_HSD$group, file = paste0(temp_out,"disper_HSD_", dd, "_postpartum.csv"), quote = F)
  # 
  # per <- adonis2(wwdist ~ Time_post_partum_day, data = wwmeta, by = "margin")
  # res <-calcOmega2_dm(per)
  # write.csv(res, file = paste0(temp_out,"adonis_test_", dd, "_postpartum.csv"), quote = F, row.names = F)
  
  #   ## Maternal_age
  # wwmeta <- wmeta %>% filter(!is.na(Maternal_age))
  # wwdist <- dist_subset(wdist, wwmeta$SampleID )
  # disper <- betadisper(wwdist, wwmeta$Maternal_age)
  # disper_ova <- anova(disper)
  # disper_HSD <- TukeyHSD(disper)
  # write.csv(disper_ova, file = paste0(temp_out,"disper_anova_", dd, "_Maternal_age.csv"), quote = F)
  # write.csv(disper_HSD$group, file = paste0(temp_out,"disper_HSD_", dd, "_Maternal_age.csv"), quote = F)
  # 
  # per <- adonis2(wwdist ~ Maternal_age, data = wwmeta, by = "margin")
  # res <-calcOmega2_dm(per)
  # write.csv(res, file = paste0(temp_out,"adonis_test_", dd, "_Maternal_age.csv"), quote = F, row.names = F)
  # 
  # ## Maternal_weight_kg
  # wwmeta <- wmeta %>% filter(!is.na(Maternal_weight_kg))
  # wwdist <- dist_subset(wdist, wwmeta$SampleID )
  # disper <- betadisper(wwdist, wwmeta$Maternal_weight_kg)
  # disper_ova <- anova(disper)
  # disper_HSD <- TukeyHSD(disper)
  # write.csv(disper_ova, file = paste0(temp_out,"disper_anova_", dd, "_Maternal_weight_kg.csv"), quote = F)
  # write.csv(disper_HSD$group, file = paste0(temp_out,"disper_HSD_", dd, "_Maternal_weight_kg.csv"), quote = F)
  # 
  # per <- adonis2(wwdist ~ Maternal_weight_kg, data = wwmeta, by = "margin")
  # res <-calcOmega2_dm(per)
  # write.csv(res, file = paste0(temp_out,"adonis_test_", dd, "_Maternal_weight_kg.csv"), quote = F, row.names = F) 
  #   ## Birth_mode
  # wwmeta <- wmeta %>% filter(!is.na(Birth_mode))
  # wwdist <- dist_subset(wdist, wwmeta$SampleID )
  # disper <- betadisper(wwdist, wwmeta$Birth_mode)
  # disper_ova <- anova(disper)
  # disper_HSD <- TukeyHSD(disper)
  # write.csv(disper_ova, file = paste0(temp_out,"disper_anova_", dd, "_Birth_mode.csv"), quote = F)
  # write.csv(disper_HSD$group, file = paste0(temp_out,"disper_HSD_", dd, "_Birth_mode.csv"), quote = F)
  # 
  # per <- adonis2(wwdist ~ Birth_mode, data = wwmeta, by = "margin")
  # res <-calcOmega2_dm(per)
  # write.csv(res, file = paste0(temp_out,"adonis_test_", dd, "_Birth_mode.csv"), quote = F, row.names = F)
  
  ## all
  wwmeta <- wmeta %>% filter(!is.na(Birth_mode), !is.na(Maternal_weight_kg),!is.na(Maternal_age), !is.na(Time_post_partum_day))
  wwdist <- dist_subset(wdist, wwmeta$SampleID )
  # disper <- betadisper(wwdist, wwmeta$Birth_mode)
  # disper_ova <- anova(disper)
  # disper_HSD <- TukeyHSD(disper)
  ...
  per <- adonis2(wwdist ~ Birth_mode + Maternal_weight_kg + Maternal_age +  Time_post_partum_day + Country, data = wwmeta, by = "margin")
  res <-calcOmega2_dm(per)
  write.csv(res, file = paste0(temp_out,"adonis_test_", dd, "_All_Country.csv"), quote = F, row.names = F)
  
  per <- adonis2(wwdist ~ Birth_mode + Maternal_weight_kg + Maternal_age +  Time_post_partum_day + Continent, data = wwmeta, by = "margin")
  res <-calcOmega2_dm(per)
  write.csv(res, file = paste0(temp_out,"adonis_test_", dd, "_All_Continent.csv"), quote = F, row.names = F)
...
for(dd in c("uu","wu","bray","jaccard")){
  ...
  g <- ggplot(wdat_center, aes(x = PC1, y = PC2, color = Continent)) + 
    ...
    geom_text_repel(aes(label = Country), color = "black", max.overlaps = 20) + 
    scale_color_manual(values = Continent_color, aesthetics = c("color","fill")) + 
    labs(x = x_lab, y = y_lab, title = "") +
    theme_bw() + theme(#aspect.ratio = 1,
      panel.background = element_rect(fill = NA),
...
load("~/Dropbox/rutgers/20220606_RUN41_milk/Data/Global.Rdata")
temp_out <- "~/Dropbox/rutgers/20220606_RUN41_milk/Data/output/global/taxa/"
...
for (taxrank in c("ASV","Species","Genus")){
  if (taxrank == "ASV"){
    ft <- otu_table(phylo_global_decontam_clean_rf@otu_table) %>% as.data.frame() %>% t()
...
dat_title <- meta_global_rf %>% count(Country) %>% 
  mutate(title = str_c(Country, " N=",n))

g_lst <- lapply(res$Country %>% unique(), function(x){
  title_x <- dat_title %>% filter(Country == x) %>% pull(title)
...
for (taxrank in c("ASV", "Species","Genus")){
  if(taxrank == "ASV"){
    ...
    taxa_temp <- taxa %>% mutate(taxa_long = str_c(Phylum,Species_l,ASV, sep = ";"))
  }else if(taxrank == "Species"){
    ft <- otu_table(phylo_global_s@otu_table) %>% as.data.frame()
    taxa_temp <- taxa %>% mutate(taxa_long = str_c(Phylum,Species_l, sep = ";"))
  }else if(taxrank == "Genus"){
    ft <- otu_table(phylo_global_g@otu_table) %>% as.data.frame()
    taxa_temp <- taxa %>% mutate(taxa_long = str_c(Phylum,Genus_l, sep = ";"))
  }
...
for (taxrank in c("ASV", "Species", "Genus")){
  ...
  ft_logra <- log10((ft + 0.5)/colSums(ft)[1])
  
  meta_global_ord <- meta_global_rf %>% arrange(Continent, Country, SampleID)
  ft_select <- ft[select_id, meta_global_ord %>% pull(SampleID)]
  if (nrow(ft_select)>50){
    ft_select <- ft_select[(rowSums(ft_select) %>% sort(.,decreasing = T))[1:50] %>% names(),]
  }
  
  wft <- ft_logra[rownames(ft_select),]
  ...
  ft_select <- ft[top_pre_id, meta_global_ord %>% pull(SampleID)]
  if (nrow(ft_select)>50){
    ft_select <- ft_select[(rowSums(ft_select) %>% sort(.,decreasing = T))[1:50] %>% names(),]
  }
  wft <- ft_logra[rownames(ft_select),]
...
for (taxrank in c("Genus", "ASV", "Species")){
  if(taxrank == "ASV"){
  ...
  #line 66 data = as.data.frame(meta_data)
  ...
  output <- ancombc2(data = phylotmp, 
                     ...
                     group = "Country", struc_zero = TRUE, neg_lb = FALSE,
                     alpha = 0.05, n_cl = 16, verbose = TRUE,
  ...
  for (dtype in c("bias_correct_log", "log")){
    ...
    wft2 = wft %>% column_to_rownames(var = "taxa_long")
    df_row_name_label = structure(rownames(wft2), names = rownames(wft2))
    meta_global_ord <- meta_global_rf %>% filter(SampleID %in% colnames(wft2)) %>%  arrange(Continent, Country, SampleID)
    wft2 <- wft2[,meta_global_ord$SampleID]
    df_col = meta_global_ord %>% dplyr::select(Continent, Country)
    column_ha = HeatmapAnnotation(df = df_col %>% dplyr::select(Continent, Country), 
                                  col = list(Continent = Continent_color,
...
dat2 <- dat %>% filter(id %in% (contam_asv %>% filter(prev=="Y") %>% pull(id))) %>% 
  left_join(taxa, by = "id") %>% 
  dplyr::select(Kingdom:ASV, prevalence, abundance)

write_csv(dat2, file = paste0(temp_out,"contam_asv_pre.csv"))
```
...
ft <- otu_table(phylo_global_g@otu_table) %>% as.data.frame() %>% 
  rownames_to_column(var = "id") %>% 
  left_join(taxa, by = "id") %>% 
  filter(str_detect(taxa_long, "Methanobacteriales")) %>% 
  dplyr::select(-c(id,Kingdom:ASV, taxa_long, Class_l:Family_l, Species_l)) %>% column_to_rownames(var = "Genus_l") %>% 
  t() %>% as.data.frame() %>% rownames_to_column(var = "SampleID") %>% 
  left_join(meta_global_rf %>% dplyr::select(SampleID, Country), by = "SampleID") 

wdat <- ft %>% 
  mutate(ra_g_Methanosphaera = g_Methanosphaera/depth,
         ra_g_Methanobacterium = g_Methanobacterium/depth,
         ra_g_Methanobrevibacter = g_Methanobrevibacter/depth,
         none = g_Methanosphaera + g_Methanobacterium + g_Methanobrevibacter) %>% 
  filter(none > 0)

write_csv(wdat, file = paste0(temp_out,"Methanogen_RA.csv"))
...
my_summarize2 <-  function(x){
  data.frame(
    ...
    mutate(
      percent.none0 = N.none0/N
    )
}

methanogen_summary <- ft %>% dplyr::select(-SampleID) %>% 
      group_by(Country) %>% 
      summarise_all(my_summarize2) %>% 
      ungroup() %>% 
      unnest(cols = c("g_Methanosphaera","g_Methanobacterium","g_Methanobrevibacter"),names_sep = ";")

write_csv(methanogen_summary, file = paste0(temp_out,"Methanogen_summary.csv"))
```

### taxa diff boxplot by country
```{r}
diff_taxa <- c("g_Acinetobacter", "g_Alistipes", "g_Bacillus", "g_Bacteroides", 
               "g_Bifidobacterium", "g_Gemella", "g_Lactobacillus","g_Muribaculaceae",
               "g_Prevotella", "g_Pseudomonas", "g_Staphylococcus","g_Streptococcus")

gg = "g_Pseudomonas"

ft <- otu_table(phylo_global_g@otu_table) %>% as.data.frame()
ft_ra <- ft/colSums(ft)[1]
ft_ra <- ft_ra %>% rownames_to_column(var = "id") %>% left_join(taxa, by = "id")

ft_log <- log10((ft + 0.5)/colSums(ft)[1])
ft_log <- ft_log %>% rownames_to_column(var = "id") %>% left_join(taxa, by = "id")
##log is better
g_lst <- list()
for (gg in diff_taxa){
  for (dtype in c("log", "ra")){
    if(dtype == "log"){
      ft <- ft_log
    }else{
      ft <- ft_ra
    }
    ft <- ft %>% filter(Genus_l == gg) %>% 
      column_to_rownames(var = "id") %>% t() %>% as.data.frame() %>% 
      rownames_to_column(var = "SampleID") 
    names(ft) <- c("SampleID","Value")
    dat <- ft %>% left_join(meta_global_rf, by = "SampleID") %>% 
      mutate(Country = factor(Country,
    ...
    res <- comp_kw_aov(wdat = dat, x = "Country", y = "Value")
    res_summary <- dat %>% dplyr::select(Value, Country) %>% 
      group_by(Country) %>% 
      summarise_all(my_summarize) %>% 
      ungroup() %>%
      unnest(cols = Value) %>% 
    ...
    g <- ggplot(dat %>% 
                  mutate(Country = factor(Country, levels= res_summary %>% arrange(desc(Med)) %>% pull(Country), ordered = T)),
                aes(Country, Value)) + 
      geom_boxplot(aes(color = Country, fill = Country), alpha = 0.6,  outlier.alpha = 0) + 
      geom_point(aes(color = Country), position = position_jitterdodge()) + 
      geom_text(data = res_summary, 
                aes(x =  Country, y = 1.15 * Max, label = KW_group)) +
      # geom_text(data = res_summary, 
      #           aes(x =  Country, y = -0.01, label = N_lab)) +
      scale_color_manual(values = Country_color, aesthetics = c("color","fill")) + 
      scale_x_discrete(labels=Country_labels) + 
      labs(x = "", y = ifelse(dtype == "log","LogRA","RA"), title = gg) +
      theme_bw() + theme(#aspect.ratio = 0.8,
        legend.position="none",
        panel.background = element_rect(fill = NA),
        strip.text = element_text(size=12,color="black",face='bold'),
    ...
    if(dtype == "log"){g_lst[[gg]] <- g}
    ggsave(filename = paste0(temp_out, gg,"_by_Country_", dtype, "_ordered.pdf"), device = cairo_pdf, 
           plot = g, width = 5, height = 5, units = "in", dpi = 300) 
  }
}

p <- g_lst[[1]]
for(i in c(2:length(g_lst))){
  p <- p + g_lst[[i]]
}
p + plot_layout(ncol = 4)
ggsave(filename = paste0(temp_out,"diff_genus_by_Country_log_ordered.pdf"), device = cairo_pdf, 
       plot = p, width = 20, height = 15, units = "in", dpi = 300) 
